...
export interface RefundPaymentInput { id?: string; merchantOrderId?: string }
export interface RefundPaymentOutput { id: string; status: PaymentStatus }
...
export class RefundPaymentUseCase {
  ...
  async execute(input: RefundPaymentInput): Promise<RefundPaymentOutput> {
    ...
    if (payment.status !== PaymentStatus.PAID) throw Object.assign(new Error('仅已支付可退款'), { status: 400 });
